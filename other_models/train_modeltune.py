...
from modeltune import model_init
# from modeltunebeta import model_init
...
def plot_hist(history, imgname):
    ...
    plt.savefig("./plots/" + imgname + "/loss.png")
    # plt.clf()
    ...
    plt.savefig("./plots/" + imgname + "/probsloss.png")
    # plt.clf()
    ...
    plt.savefig("./plots/" + imgname + "/timeloss.png")
    plt.clf()
    plt.close()
...
if __name__ == "__main__":
    ...
    num_epochs = 30
    num_examples_per_epoch = 25000  # If use_generator == 0, number of examples that are loaded, else this helps define steps_per_epoch in the training
    num_examples_per_epoch = (num_examples_per_epoch // batch_size) * batch_size
    # Cannot be > 200 if use_generator == 0, RAM will crash
    depth = 5
    hidden_size = 32
    lr = 0.001
    myLosses = {'prediction_probs': "categorical_crossentropy", 'prediction_time': 'mse'}
    ...
    model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
        ...
        monitor='prediction_probs_loss',
        mode='min',
        save_freq='epoch',
        save_best_only=True)
    early_stopping = tf.keras.callbacks.EarlyStopping(monitor='prediction_probs_loss', patience=3, mode='min',
                                                      restore_best_weights=True)
    reducer = tf.keras.callbacks.ReduceLROnPlateau(monitor='prediction_probs_loss', factor=0.1, patience=3, verbose=0,
                                                   mode='auto',
                                                   min_delta=0.0001, cooldown=0, min_lr=0)
    for num in [1]:
        for lr in ratelist:
            for hidden_size in hiddenlist:
                # Initialize model with inputs, and additionnal methods for training
                tf.keras.backend.clear_session()
                model = model_init(num_atoms, num_atom_features, num_pairs_features, max_num_of_bonds, depth, lr,
                                   hidden_size,nbdense)
                if num ==1:
                    lrscheduler = tf.keras.callbacks.LearningRateScheduler(scheduler)
                else:
                    lrscheduler = tf.keras.callbacks.LearningRateScheduler(schedulerbis)
                callbacks_list = [ early_stopping, model_checkpoint_callback]
                ...
                model.compile(optimizer=optim, loss=myLosses, loss_weights=myLossesWeights, metrics=['accuracy'])
                # Fit the model with the appropriated  data pipeline method,so the full dataset does not need to fit into memory.
                if use_generator == 1:
                    history = model.fit(
                        data.repeat().batch(batch_size=batch_size).prefetch(tf.data.experimental.AUTOTUNE),
                        epochs=num_epochs, steps_per_epoch=int(num_examples_per_epoch / batch_size),
                        validation_data=(
                        [X_input_atom_val, X_input_pairs_val, X_atom_graph_val, X_mask_val, X_extract_pairs_val],
                        [Y_val, Y_time_val]), callbacks=callbacks_list)
                else:
                    history = model.fit([X_input_atom, X_input_pairs, X_atom_graph, X_mask, X_extract_pairs],
                                        [Y, Y_time],
                                        batch_size=batch_size, epochs=num_epochs,
                                        validation_data=(
                                        [X_input_atom_val, X_input_pairs_val, X_atom_graph_val, X_mask_val,
                                         X_extract_pairs_val], [Y_val, Y_time_val]), callbacks=callbacks_list)
                # Save the model
                savedir = "16112020modeltune_scheduler" + str(num) + "_hidden" + str(hidden_size) + "_optim-" + str(opt)
                model.save("saved_model/" + savedir)
                ...
                plot_molecules_of_interest(molecules_per_frame, molecule_list, time_range, molecules_per_frame_MD,
                                           molecule_list_MD, time_range_MD * 0.012, molecules_of_interest, savedir)
