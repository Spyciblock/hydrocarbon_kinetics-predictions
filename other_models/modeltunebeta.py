...
def model_init(num_atoms, num_atom_features, num_pairs_features, max_num_of_bonds, depth, lr, hidden_size,
                       nbdense):
    ...
    input_pairs = tf.keras.Input(shape=[num_atoms * (num_atoms - 1) // 2,
                                        num_pairs_features])  # shape = num_atoms, num_atoms, num_pair_atoms_features
    atom_graph = tf.keras.Input(shape=[num_atoms, max_num_of_bonds, 2],
                                dtype=tf.int32)  # shape = num_atoms, max_num_of_bonds, 2
    mask = tf.keras.Input(shape=[num_atoms, max_num_of_bonds, 1],
                          dtype=tf.float32)  # shape = num_atoms, max_num_of_bonds, 1
    ...
    atom_features = tf.keras.layers.Dense(hidden_size, activation='relu', kernel_regularizer="l1_l2", name="Dense0")(
        input_atom)
    ...
    for i in range(depth):
        ...
        fatom_nei = tf.gather_nd(atom_features,
                                 atom_graph)  # shape [num_atoms,  max_number_of_bonds, num_atom_features]
    ...
    new_input_pairs = tf.keras.layers.Dense(hidden_size, activation="relu", kernel_regularizer="l1_l2", name="Dense6")(
        input_pairs)  # shape = num_atoms,num_atoms,hidden_size
    if nbdense > 1:
        new_input_pairs = tf.keras.layers.Dense(hidden_size, activation="relu", kernel_regularizer="l1_l2",
                                                name="Dense6b")(new_input_pairs)
        new_input_pairs = tf.keras.layers.Dense(hidden_size, activation="relu", kernel_regularizer="l1_l2",
                                                name="Dense6c")(new_input_pairs)

    print("new_input_pairs shape=", new_input_pairs.shape)

    # Gather all informations of the generated atoms features
    concat = tf.keras.layers.Concatenate(axis=-1)(
        [atom_features_1, atom_features_2])  # shape = num_atoms,num_atoms,hidden_size
    concat = tf.gather_nd(concat, extract_pairs)
    midlayer = tf.keras.layers.Dense(hidden_size, activation="relu", kernel_regularizer="l1_l2", name="Dense7")(
        concat)  # shape = num_atoms,num_atoms,hidden_size
    ...
    endput0 = tf.keras.layers.Dense(hidden_size, activation=None, kernel_regularizer="l1_l2", name="Dense8")(
        concatenation_atom_and_pair_features)  # shape = num_atoms,num_atoms,hidden_size
    print("endput shape=", endput0.shape)
    activ = tf.keras.layers.LeakyReLU(alpha=0.3)  # use leaky relu
    endput = activ(endput0)
 
    endconv = tf.keras.layers.Conv1D(48, 1, padding="valid", activation=None)(endput)
    print("endconv shape=", endconv.shape)
    afterendconv = tf.keras.layers.LeakyReLU(alpha=0.8)
    finalendconv = afterendconv(endconv)
    ab = tf.keras.layers.Dense(48, activation="relu", kernel_regularizer="l1_l2", name="Dense10")(finalendconv)
    # print("ab shape=", ab.shape)
    ab2 = tf.keras.layers.Dense(32, activation="relu", name="Dense10a")(ab)
    # print("ab2 shape=", ab2.shape)

    endconv3 = tf.keras.layers.Conv1D(2, 1, padding="valid", activation=None)(ab2)
    print("endconv3 shape=", endconv3.shape)
    layer0 = tf.keras.layers.LeakyReLU(alpha=0.8)  # use leaky relu
    endconv3 = layer0(endconv3)
    midendconv = tf.keras.layers.Dense(hidden_size, activation="relu", kernel_regularizer="l1_l2", name="Dense11")(
        endconv3)
    print("midendconv shape=", midendconv.shape)

    endconv3bis = tf.keras.layers.Conv1D(1, 1, padding="valid", activation=None)(midendconv)
    print("endconv3bis shape=", endconv3bis.shape)
    layer = tf.keras.layers.LeakyReLU(alpha=0.1)
    endconv3bis = layer(endconv3bis)
    finalconcat = tf.keras.layers.Dense(32, activation="relu", kernel_regularizer="l1_l2", name="Dense10")(endconv3bis)
    print("finalconcat shape=", finalconcat.shape)
    ...
    preds = tf.reduce_sum(finalconcat, axis=-1)# with endput or finalconcat
    print("preds shape=", preds.shape)
    preds_probs = tf.keras.layers.Softmax(name='prediction_probs')(preds)
    preds_time = tf.keras.layers.Flatten()(preds)
    print("preds time first shape=", preds_time.shape)
    ...
    # preds_time = tf.keras.layers.Dense(1, kernel_initializer='ones', trainable=False, name='prediction_time')(preds_time)
    ...
    return tf.keras.Model(inputs=[input_atom, input_pairs, atom_graph, mask, extract_pairs],
                          outputs=[preds_probs, preds_time])
